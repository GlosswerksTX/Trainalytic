...
const App = () => {
  ...
  // Pulse Check seller-brand state
  ...
  const onWizardFinish = ({ config: newConfig, skills, training, addedTrainees, loadSamples }) => {
    setConfig({ ...newConfig });
    if (loadSamples) {
      setTrainees(SAMPLE_TRAINEES);
    } else {
      // Map added trainees to placeholder records
        ...
        <div className="app-main">
          ...
          <div className="app-body">
            <div className="app-body-inner">
              ...
              {page === 'settings' && <SettingsPage config={config} setConfig={setConfig} onRunSetup={() => navigate('setup')} onDisconnect={() => { SheetAPI.clearUrl(); setSheetUrl(null); setDemoMode(true); setLoadError(null); showToast('Disconnected. Your sheet is untouched.', 'info'); setConfig(c => ({ ...c, ...SAMPLE_CONFIG })); setTrainees(SAMPLE_TRAINEES); setTrainers(SAMPLE_TRAINERS); }} sheetUrl={sheetUrl} />}
            </div>
          </div>
        </div>
      </div>
