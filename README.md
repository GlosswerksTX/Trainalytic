# Pulse Check — Employee Training Tracker (Dashboard)
...
1. Buy a domain like `pulsecheck.app` on Cloudflare or Namecheap.
2. In your domain registrar, add a CNAME record pointing `app.pulsecheck.app` → `YOUR-USERNAME.github.io`.
3. In your repo: Settings → Pages → Custom domain → enter `app.pulsecheck.app` → Save → check "Enforce HTTPS" once Pages validates it.
...
assets/
  pulse-check-logo.png     ← seller brand logo
```
...
Buyer questions go to hello@pulsecheck.app (or whatever you set up).
